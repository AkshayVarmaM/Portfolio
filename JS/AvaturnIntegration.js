...
function setupScene(gltf) {
    ...
    const pushClip = THREE.AnimationClip.findByName(clips, 'pushing');
    const armStrechingClip = THREE.AnimationClip.findByName(clips, 'armStreching');
    const waveAction = mixer.clipAction(waveClip);
    const walkAction = mixer.clipAction(walkClip);
    const pushAction = mixer.clipAction(pushClip);
    const armStrechingAction = mixer.clipAction(armStrechingClip);
    ...
    container.addEventListener('mousedown', (ev) => {
        ...
        if (intersections.length > 0) {
            ...
            isStumbling = true;
            armStrechingAction.reset();
            armStrechingAction.play();
            waveAction.crossFadeTo(armStrechingAction, 0.3);

            setTimeout(() => {
                waveAction.reset();
                waveAction.play();
                armStrechingAction.crossFadeTo(waveAction, 1);
